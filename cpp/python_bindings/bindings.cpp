...
class Index {
 public:
    ...
    /*
     * DHNSW: insert with a per-point degree (M) and construction ef, as
     * produced by the RP-KNN density estimate on the Python side. The density
     * estimation itself is deliberately not ported -- it stays in sklearn and
     * arrives here as two arrays, one entry per row of `input`.
     */
    void addItemsDynamic(py::object input, py::object m_per_node, py::object ef_per_node,
                         size_t m0 = 0, py::object ids_ = py::none(), int num_threads = -1) {
        py::array_t<dist_t, py::array::c_style | py::array::forcecast> items(input);
        py::array_t<size_t, py::array::c_style | py::array::forcecast> ms(m_per_node);
        py::array_t<size_t, py::array::c_style | py::array::forcecast> efs(ef_per_node);
        auto buffer = items.request();
        if (num_threads <= 0)
            num_threads = num_threads_default;
        ...
        if (features != dim)
            throw std::runtime_error("Wrong dimensionality of the vectors");
        if ((size_t)ms.size() != rows || (size_t)efs.size() != rows)
            throw std::runtime_error("m_per_node and ef_per_node must have one entry per row");

        if (rows <= (size_t)num_threads * 4)
            num_threads = 1;

        std::vector<size_t> ids = get_input_ids_and_check_shapes(ids_, rows);
        const size_t *m_data = ms.data();
        const size_t *ef_data = efs.data();

        {
            ...
            if (!ep_added) {
                ...
                appr_alg->addPoint((void *)vector_data, (size_t)id, -1, m_data[0], ef_data[0], m0);
                start = 1;
                ep_added = true;
            }
            ...
            if (normalize == false) {
                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
                    size_t id = ids.size() ? ids.at(row) : (cur_l + row);
                    appr_alg->addPoint((void *)items.data(row), (size_t)id, -1,
                                       m_data[row], ef_data[row], m0);
                    });
            } else {
                std::vector<float> norm_array(num_threads * dim);
                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    normalize_vector((float *)items.data(row), (norm_array.data() + start_idx));
                    size_t id = ids.size() ? ids.at(row) : (cur_l + row);
                    appr_alg->addPoint((void *)(norm_array.data() + start_idx), (size_t)id, -1,
                                       m_data[row], ef_data[row], m0);
                    });
            }
            cur_l += rows;
    ...
    size_t getTotalEdgeCount() const {
        return appr_alg->getTotalEdgeCount();
    }
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("add_items_dynamic",
            &Index<float>::addItemsDynamic,
            py::arg("data"),
            py::arg("m_per_node"),
            py::arg("ef_per_node"),
            py::arg("m0") = 0,
            py::arg("ids") = py::none(),
            py::arg("num_threads") = -1)
        .def("get_total_edge_count", &Index<float>::getTotalEdgeCount)
